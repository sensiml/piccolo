...
peak_to_peak_segmenter_contracts = {
    "input_contract": [
...
def peak_peak_detection(input_data, params, idx):
    ...
    max_val = MIN_INT
    for col in cols:
        tmp_val = np.max(input_data[col].loc[idx : idx + params["start_window"]])
        if max_val < tmp_val:
            max_col = col
            max_val = tmp_val

    start_index = idx + np.argmax(
        input_data[max_col].loc[idx : idx + params["start_window"]]
    )

    if start_index + params["max_segment_length"] >= len(input_data):
        return None, None
    end_index = np.argmax(
        input_data[max_col].loc[
            start_index
