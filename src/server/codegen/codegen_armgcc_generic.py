...
class ARMGCCGenericCodeGenerator(KnowledgePackCodeGeneratorBase):
    def __init__(
        self,
        knowledgepacks,
        uuid,
        ...
        super(ARMGCCGenericCodeGenerator, self).__init__(
    ...
    def generate(self, build_type="lib"):
        ...
        if self.is_tensorflow(classifier_types):
            ...
            if "A53" in proc.display_name.upper() or "A72" in proc.display_name.upper():
                # TODO: This should  needs to select the right version of tensorflow for the platform right now it is the one
                # compiled for a72
                platform = "cortex_a_generic"
