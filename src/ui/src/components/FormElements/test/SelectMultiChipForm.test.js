...
describe("<SelectMultiChip/>", () => {
  ...
  const defaultValue = [options[0].value, options[1].value, options[2].value];
  ...
  it("Should call onChange props with all options", async () => {
    const handleChange = jest.fn();
    renderWithTheme(
      <SelectMultiChip id={id} name={name} onChange={handleChange} options={options} />,
    );
    ...
    expect(handleChange.mock.calls[0]).toEqual([name, []]);
    expect(handleChange.mock.calls[1]).toEqual([name, [options[0].value]]);
    expect(handleChange.mock.calls[2]).toEqual([name, [options[0].value, options[1].value]]);
    expect(handleChange.mock.calls[3]).toEqual([
      name,
      [options[0].value, options[1].value, options[2].value],
    ]);
  });
});
