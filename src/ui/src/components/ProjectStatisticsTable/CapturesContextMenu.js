...
export default function CapturesContextMenu({ closeAction, menuActions, openMenu, anchor }) {
  ...
  const handleSelectAllInTable = (event) => {
    if (menuActions) menuActions.selectAllInTable(event);
    handleClose(event);
  };

  const handleUnSelectAllInTable = (event) => {
    if (menuActions) menuActions.unSelectAllInTable(event);
    handleClose(event);
  };
