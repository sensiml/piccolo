...
import StepDataTransform from "../../domain/PipelineStepsDataFactory/StepDataTransform";
...
const TRANSFORM_LIST = [
  ...
  PIPELINE_STEP_TYPES.AUGMENTATION,
...
describe("StepDataTransform", () => {
  ...
  pipelineSteps = state.autoML.pipelineHierarchyRules.data.filter(el => TRANSFORM_LIST.includes(el.type));

  pipelineSteps.forEach(step => {
    describe(`${step.type} ${step.subtype}`, () => {
      let transformDataClass;
      let inputData;
      transformDataClass = new StepDataTransform({
        state,
        type: step.type,
      ...
      describe("Convert input contract types to dynamic form type", () => {
          ...
          inputData.fieldset.forEach(formElement => {
            ...
            if ([FORM_TYPES.FORM_MULTI_SELECT_TYPE].includes(formElement.type)) {
              ...
              it(`${formElement.name} -- FORM_MULTI_SELECT_TYPE elements have default values`, () => {
