...
describe("StepDataFeatureGenerator", () => {
  let store;
  let state;
  let pipelineSteps = [];
  store = setupMockStore();
  state = store.getState();
  pipelineSteps = state.autoML.pipelineHierarchyRules.data.filter(el => el.type === PIPELINE_STEP_TYPES.FEATURE_GENERATOR);

  pipelineSteps.forEach(step => {
    describe(`${step.type} ${step.subtype}`, () => {
      let transformDataClass;
      let inputData;
      transformDataClass = new StepDataFeatureGenerator({
        state,
        type: step.type,
      ...
      describe("Convert input contract types to dynamic form type", () => {
        inputData.featureList.forEach(feature => {
          feature.inputContract.forEach(formElement => {
