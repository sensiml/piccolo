...
int32_t fg_roc_zero_crossing_rate(kb_model_t *kb_model, int16_data_t *cols_to_use, float_data_t *params, FLOAT *pFV)
{
...
#if SML_DEBUG
	if (!kb_model || !cols_to_use || cols_to_use->size <= 0 || !params || params->size != 0 || kb_model->sg_length <= 0 || !pFV)
		return 0;
#endif
	...
	for (icol = 0; icol < cols_to_use->size; icol++)
	{
		...
		float num_crossings = (float)calculate_zc_with_xor(sortedData, kb_model->sg_length);
