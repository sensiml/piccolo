...
static void calc_harmonic_product_spectrum_peak(int16_t *input_data, int32_t len, int32_t harmonic_coeff, FLOAT *pFV)
{
    ...
    int32_t ndx = 0;
    int32_t val;
    int32_t max_downsample_steps = NUM_FFTR_CMPX / harmonic_coeff;

    data = fftr_rm_as(input_data, len);

    for (int32_t i = 0; i < NUM_FFTR_CMPX; i++)
    {
        input_data[i] = SQRT(data[i].real * data[i].real + data[i].imag * data[i].imag);
    }

    for (int32_t i = 1; i < max_downsample_steps; i++)
    {

        val = input_data[i];
        for (int32_t j = 2; j < harmonic_coeff; j++)
        {
            val *= input_data[i * j];
        }
