...
{
#define FG_SHAPE_MEDIAN_DIFFERENCE_NUM_PARAMS 1
#define FG_SHAPE_MEDIAN_DIFFERENCE_CENTER_RATIO_PARAM_IDX 0

#if SML_DEBUG
    if (!kb_model || !cols_to_use || cols_to_use->size <= 0 || !params || params->size != FG_SHAPE_MEDIAN_DIFFERENCE_NUM_PARAMS || kb_model->sg_length <= 0 || !pFV)
    {
        return 0;
    }
#endif // SML_DEBUG
    ...
    float center_ratio = params->data[FG_SHAPE_MEDIAN_DIFFERENCE_CENTER_RATIO_PARAM_IDX];
