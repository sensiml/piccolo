...
class FGShapeMedianDifference : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(FGShapeMedianDifference, kb_model_NULL_param_test)
{
    // kb_model is NULL
    ret = fg_shape_median_difference(NULL, &cols_to_use, &params, pFV);
    ASSERT_EQ(0, ret);
}

TEST_F(FGShapeMedianDifference, NULL_cols_to_use_param_test)
{
    ret = fg_shape_median_difference(&kb_model, NULL, &params, pFV);
    ASSERT_EQ(0, ret);
}


TEST_F(FGShapeMedianDifference, NULL_feature_vector_test)
{
    ret = fg_shape_median_difference(&kb_model, &cols_to_use, &params, NULL);
    ASSERT_EQ(0, ret);
}

TEST_F(FGShapeMedianDifference, generate_features_test)
{
    params.data[0] = .5;
    ret = fg_shape_median_difference(&kb_model, &cols_to_use, &params, pFV);
...
TEST_F(FGShapeMedianDifference, generate_features_threshold_75_test)
{
    params.data[0] = .75;
    ret = fg_shape_median_difference(&kb_model, &cols_to_use, &params, pFV);
    ASSERT_EQ(num_cols, ret);
...
TEST_F(FGShapeMedianDifference, generate_features_threshold_25_test)
{
    params.data[0] = .25;
    ret = fg_shape_median_difference(&kb_model, &cols_to_use, &params, pFV);
    ASSERT_EQ(num_cols, ret);
