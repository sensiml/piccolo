...
class FGCrossMedianDifferenceTest : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(FGCrossMedianDifferenceTest, median_difference_1_minus_2)
{
    printf("About To Run Test");
    cols_to_use.data[0] = 0; //use the first column
    cols_to_use.data[1] = 1; //use the first column

    ret = fg_cross_column_median_difference(&kb_model, &cols_to_use, &params, pFV);
    ASSERT_EQ(1, ret);
    float tolerance = 0.001f;
    printf("%f", pFV[0]);

    EXPECT_NEAR(pFV[0], -10.5, tolerance);
}

TEST_F(FGCrossMedianDifferenceTest, median_difference_2_minus_1)
{
    printf("About To Run Test");
