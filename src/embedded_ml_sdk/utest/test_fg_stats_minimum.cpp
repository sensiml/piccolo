...
class FGMinimumTest : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(FGMinimumTest, kb_model_NULL_param_test)
{
    // kb_model is NULL
    ret = fg_stats_minimum(NULL, &cols_to_use,
                           &params, pFV);
    ASSERT_EQ(0, ret);
    
    ret = fg_stats_minimum(&kb_model, &cols_to_use,
                           &params, NULL);
    ASSERT_EQ(0, ret);
}


TEST_F(FGMinimumTest, return_number_of_cols_test)
{
...
TEST_F(FGMinimumTest, return_value_of_each_cols_test)
{
    fg_stats_minimum(&kb_model, &cols_to_use,
                     &params, pFV);
