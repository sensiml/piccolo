...
class FGTAreaAbsoluteAreaHighFrequency : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(FGTAreaAbsoluteAreaHighFrequency, ring_size_vs_test_data_len_test)
{
    ASSERT_GT(RB_SIZE, NUM_ROWS_TEST1);
    ASSERT_GT(RB_SIZE, NUM_ROWS_TEST2);
}

TEST_F(FGTAreaAbsoluteAreaHighFrequency, kb_model_NULL_test)
{

    int ret = fg_area_absolute_area_high_frequency(NULL, &cols_to_use, &params, pFV);

    ASSERT_EQ(0, ret);
}

TEST_F(FGTAreaAbsoluteAreaHighFrequency, num_cols_zero_test)
{

    cols_to_use.size = 0; // test, expect 0 as return value
    cols_to_use.size=0;
    int ret = fg_area_absolute_area_high_frequency(&kb_model, &cols_to_use, &params, pFV);

    ASSERT_EQ(ret, 0);
}

TEST_F(FGTAreaAbsoluteAreaHighFrequency, cols_to_use_NULL_test)
{
    cols_to_use.size = 0; // test, expect 0 as return value
    int ret = fg_area_absolute_area_high_frequency(&kb_model, NULL, &params, pFV);

    ASSERT_EQ(ret, 0);
}

TEST_F(FGTAreaAbsoluteAreaHighFrequency, pFV_NULL_test)
{

    int ret = fg_area_absolute_area_high_frequency(&kb_model, &cols_to_use, &params, NULL);

    ASSERT_EQ(ret, 0);
}

TEST_F(FGTAreaAbsoluteAreaHighFrequency, calculation_dataset1_test)
{
...
TEST_F(FGTAreaAbsoluteAreaHighFrequency, calculation_dataset2_test)
{

    //cols_to_use.data[NUM_COLS] = { 0, 1, 2 };
