...
class SGFilterVadWebRTCTest : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(SGFilterVadWebRTCTest, kb_model_NULL_param_test)
{
    sg_filter_vad_webrtc_init();

    // kb_model is NULL
    ret = sg_filter_vad_webrtc(NULL, &cols_to_use, &params);
    printf("\n\nkb_model_NULL_param_test = %d\n\n", ret);
    ASSERT_EQ(-1, ret);
}

TEST_F(SGFilterVadWebRTCTest, num_cols_zero_param_test)
{
    sg_filter_vad_webrtc_init();

    cols_to_use.size = 0; // test, expect 0 as return value
    cols_to_use.size = 0;
    ret = sg_filter_vad_webrtc(&kb_model, &cols_to_use, &params);
    ASSERT_EQ(-1, ret);
}

TEST_F(SGFilterVadWebRTCTest, cols_to_use_NULL_param_test)
{
    sg_filter_vad_webrtc_init();

    cols_to_use.size = 0;
    // cols_to_use = NULL test, expect 0 as return value
    ret = sg_filter_vad_webrtc(&kb_model, NULL, &params);
    ASSERT_EQ(-1, ret);
}

TEST_F(SGFilterVadWebRTCTest, params_NULL_param_test)
{
    sg_filter_vad_webrtc_init();

    // pFV = NULL test, expect 0 as return value
    ret = sg_filter_vad_webrtc(&kb_model, &cols_to_use,
                               NULL);
    ASSERT_EQ(-1, ret);
}

TEST_F(SGFilterVadWebRTCTest, wrong_num_params_param_test)
{
    sg_filter_vad_webrtc_init();
...
TEST_F(SGFilterVadWebRTCTest, return_value_of_each_cols_test)
{
    ...
    params.data[0] = 15; // threshold
    params.data[1] = 17; // buffer size
    params.size =2;
    ret = sg_filter_vad_webrtc(&kb_model, &cols_to_use, &params);
...
TEST_F(SGFilterVadWebRTCTest, return_value_of_speech_detect)
{
    int count = 0;
    int speech_detect = 0;
    ...
    while(speech_detect == 0)
    {
        ...
        speech_detect = sg_filter_vad_webrtc(&kb_model, &cols_to_use, &params);
