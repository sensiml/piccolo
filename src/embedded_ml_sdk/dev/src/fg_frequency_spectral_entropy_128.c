...
static FLOAT calc_spectral_entropy_128(int16_t *input_data, int32_t len)
{
	int32_t i;
	int32_t sum = 0;
	FLOAT entropy = 0.0f;
	...
	data = fftr_128_rm_as(input_data, len);
	if (sum == 0)
		sum = 1;

	// printf("val = [\n");

	for (i = 0; i < NUM_FFTR_128_CMPX; i++)
	{
...
int32_t spectral_entropy_128(kb_model_t *kb_model, int16_data_t *cols_to_use, FLOAT *params, int32_t num_params, FLOAT *pFV)
{
	...
	for (icol = 0; icol < cols_to_use->size; icol++)
	{
		...
		pFV[icol] = calc_spectral_entropy_128(sortedData, kb_model->sg_length);
	}
