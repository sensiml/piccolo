...
static void dump_FFTR(int16_t *pData)
{
    ...
    for (idx = 0; idx < NUM_FFTR_128; idx += 2)
    {
...
struct compx_int16_t *fftr_128(int16_t *input_data, int32_t len)
{
    int32_t rShifts;
    int32_t idx;

    // check inputs:
    if ((!input_data) || (len < 8) || (len > NUM_FFTR_128))
    {
        return NULL;
    }
    ...
    if (autoScale)
    {
        autoscale_data_int(input_data, NUM_FFTR_128);
        autoScale = false;
#ifdef DBG_UTEST
        printf("\nAutoscaled data:\n");
        dump_data_int(input_data, len);
    ...
    if (len < NUM_FFTR_128)
    {
        for (idx = len; idx < NUM_FFTR_128; idx++)
        {
            input_data[idx] = 0;
        }
    }
...
struct compx_int16_t *fftr_128_as(int16_t *input_data, int32_t len)
{
    struct compx_int16_t *pRet;

    autoScale = true;
    removeMean = false;
    pRet = fftr_128(input_data, len);
    return pRet;
}

// This removes the mean, applies Hanning, and then auto-scales:
struct compx_int16_t *fftr_128_rm_as(int16_t *input_data, int32_t len)
{
    struct compx_int16_t *pRet;
